...
import { excludeLearningRecordServer } from "@/lib/testnet-monitor/learning-records-server";
...
export async function POST(request: Request) {
  try {
    ...
    const updated = await excludeLearningRecordServer(body.learningRecordId);
    if (!updated) {
      return NextResponse.json(
    ...
    const message =
      error instanceof Error ? error.message : "Exclude learning record failed";
