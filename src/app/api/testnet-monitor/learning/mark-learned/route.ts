...
import { markLearningRecordLearnedServer } from "@/lib/testnet-monitor/learning-records-server";
...
export async function POST(request: Request) {
  try {
    ...
    const updated = await markLearningRecordLearnedServer(body.learningRecordId);
    if (!updated) {
      return NextResponse.json(
    ...
    const message =
      error instanceof Error ? error.message : "Mark learned failed";
