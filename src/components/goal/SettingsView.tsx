    ...
    >
      ...
      <section className="rounded-xl border border-zinc-800/80 bg-zinc-950/60 p-5">
        ...
        <ul className="mt-2 space-y-1.5 text-xs text-zinc-400">
          ...
          <li>
            Recommendation:{" "}
            <span className="text-zinc-300">{m.binanceTestnet.recommendation}</span>
          </li>
          <li>
            API key: {m.binanceTestnet.apiKeyPresent ? "present" : "missing"} · Secret:{" "}
            {m.binanceTestnet.apiSecretPresent ? "present" : "missing"}
          </li>
          <li>
            Proxy: {m.binanceTestnet.proxyEnabled ? m.binanceTestnet.proxyProvider ?? "on" : "off"}
            {m.binanceTestnet.proxyUrlConfigured ? "" : " (URL not configured)"}
          </li>
          <li>
            Base URL: <span className="font-mono text-zinc-300">{m.binanceTestnet.baseUrl || "—"}</span>
          </li>
          <li>
            Live trading:{" "}
